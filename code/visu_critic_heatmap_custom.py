...
result_dir = os.path.join('logs', eval_conf.exp_name, f'visu_critic_heatmap_custom-model_epoch_{eval_conf.model_epoch}-{eval_conf.result_suffix}')
...
# setup camera
cam = Camera(env, dist=0.5, random_position=True)
...
# load shape
object_urdf_fn = '../data/grasp/yellow_cup/model.urdf'
...
cur_qpos = env.get_object_qpos() # 返回操作物体的关节角度

# simulate some steps for the object to stay rest 等待仿真更新
still_timesteps = 0
wait_timesteps = 0
...
while still_timesteps < 5000 and wait_timesteps < 20000:
    ...
    for c in env.scene.get_contacts():
        ...
        if invalid_contact:
            break
    if np.max(np.abs(cur_new_qpos - cur_qpos)) < 1e-6 and (not invalid_contact): # 有接触并且
        still_timesteps += 1
    else:
        still_timesteps = 0
