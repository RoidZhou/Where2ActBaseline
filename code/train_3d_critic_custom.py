...
from data_custom import SAPIENVisionDataset
import utils
...
# python train_3d_critic_custom.py --exp_suffix train_3d_critic_custom  --model_version model_3d_critic --primact_type grasp --data_dir_prefix ../data/gt_data --offline_data_dir ../data/grasp_train_data --val_data_dir ../data/grasp_train_data --val_data_fn data_tuple_list.txt --train_shape_fn ../grasp_stats/train_data_list.txt --ins_cnt_fn ../grasp_stats/ins_cnt_15cats.txt --buffer_max_num 10000 --num_processes_for_datagen 6 --num_interaction_data_offline 50 --num_interaction_data 1 --sample_succ --overwrite
...
def train(conf, train_shape_list, train_data_list, val_data_list, all_train_data_list):
    ...
    datagen = GraspDataGen(conf.num_processes_for_datagen, conf.flog)
    ...
    for epoch in range(start_epoch, conf.epochs):
        ...
        if conf.sample_succ:
            ...
            sample_succ_list = []
            sample_succ_dirs = []
            cur_sample_succ_dir = os.path.join(conf.data_dir, 'epoch-%04d_sample-succ' % epoch) # ../data/gt_data/exp-model_3d_critic-pushing-Faucet-train_3d_critic/epoch-%04d_sample-succ
                    ...
                    for i in range(conf.batch_size):
                        ...
                        if ('sample-succ' not in ss_cur_dir[i]) and (ss_is_original[i]) and (ss_cur_dir[i] not in sample_succ_dirs) \
                            ...
                            sample_succ_list.append((conf.offline_data_dir, str_cur_dir1, str_cur_dir2, \
                                    ss_cur_dir[i].split('/')[-1], cur_sample_succ_dir, X, Y)) # ../data/gt_data-train_10cats_train_data-pushing  /149_Faucet_1_0_pushing_57 /epoch-%04d_sample-succ
...
def forward(batch, data_features, network, conf, \
    ...
    # prepare input
    input_pcs = torch.cat(batch[data_features.index('pcs')], dim=0).to(conf.device)     # B x 3N x 3 = [32, 10000, 3]
    ...
    batch_size = input_pcs.shape[0]
                                           # (32, 1)     (32, 10000)                               (320000)
    input_pcid1 = torch.arange(batch_size).unsqueeze(1).repeat(1, conf.num_point_per_shape).long().reshape(-1)  # BN = （320000）, conf.num_point_per_shape = 10000
                                                              # (32, 10000)        （320000）
    input_pcid2 = furthest_point_sample(input_pcs, conf.num_point_per_shape).long().reshape(-1)      #对点云进行最远距离采样 ,输出点云索引      # BN = （320000）
    input_pcs = input_pcs[input_pcid1, input_pcid2, :].reshape(batch_size, conf.num_point_per_shape, -1) # 利用新的索引和点云坐标重组点云张量， [32, 10000, 3]
    input_pxids = input_pxids[input_pcid1, input_pcid2, :].reshape(batch_size, conf.num_point_per_shape, -1) # 利用新的索引和点云坐标重组点云像素张量， [32, 10000, 3]
                    # [32, 30000]          [320000]            [32, 30000]
    input_movables = input_movables[input_pcid1, input_pcid2].reshape(batch_size, conf.num_point_per_shape) # 在input_movables中取出input_pcid1 和 input_pcid2 索引对应的值,组成新的input_movables mask
    ...
    gripper_img_target = torch.cat(batch[data_features.index('gripper_img_target')], dim=0).to(conf.device)     # B x 3 x H x W = [32, 3, 448, 448]
...
if __name__ == '__main__':
    ...
    # make exp_name
    conf.exp_name = f'exp-{conf.model_version}-{conf.primact_type}-{conf.category_types}-{conf.exp_suffix}' # exp-model_3d_critic-pushing-Faucet-train_3d_critic
    ...
    conf.exp_dir = os.path.join(conf.log_dir, conf.exp_name) # logs/exp-model_3d_critic-pushing-Faucet-train_3d_critic
    ...
    if not conf.resume:
        ...
        os.mkdir(os.path.join(conf.exp_dir, 'ckpts')) # logs/exp-model_3d_critic-pushing-Faucet-train_3d_critic/ckpts
        if not conf.no_visu:
            os.mkdir(os.path.join(conf.exp_dir, 'val_visu'))

    # prepare data_dir
    conf.data_dir = conf.data_dir_prefix + '-' + conf.exp_name # ../data/gt_data/exp-model_3d_critic-pushing-Faucet-train_3d_critic
    ...
    if not conf.resume:
        os.mkdir(conf.data_dir) # ../data/gt_data-exp-model_3d_critic-grasp-None-train_3d_critic_custom
    ...
    # save config
    if not conf.resume:
        torch.save(conf, os.path.join(conf.exp_dir, 'conf.pth')) # exp-model_3d_critic-pushing-Faucet-train_3d_critic

    # file log
    if conf.resume:
        flog = open(os.path.join(conf.exp_dir, 'train_log.txt'), 'a+')
    else:
        flog = open(os.path.join(conf.exp_dir, 'train_log.txt'), 'w') # exp-model_3d_critic-pushing-Faucet-train_3d_critic/train_log.txt
    conf.flog = flog
    ...
    with open(conf.train_shape_fn, 'r') as fin: # ../stats/train_10cats_train_data_list.txt
        for l in fin.readlines():
            category = l.rstrip().split()[0]
            conf.category_types.append((category))
    utils.printout(flog, 'category_types: %s' % str(conf.category_types))

    # read cat2freq
    # conf.cat2freq = dict()
    # with open(conf.ins_cnt_fn, 'r') as fin: # ../stats/ins_cnt_15cats.txt
    #     for l in fin.readlines():
    ...
    with open(os.path.join(conf.offline_data_dir, 'data_tuple_list.txt'), 'r') as fin: # ../data/gt_data-train_10cats_train_data-pushing/data_tuple_list.txt
    ...
    if conf.resume:
        ...
        train_data_list = []
        for item in all_train_data_list:
            if int(item.split('_')[-1]) < conf.num_interaction_data_offline: # 最大为50
                train_data_list.append(item)
        utils.printout(flog, 'len(train_data_list): %d' % len(train_data_list))
    
    with open(os.path.join(conf.val_data_dir, conf.val_data_fn), 'r') as fin: # ../data/gt_data-train_10cats_test_data-pushing/data_tuple_list.txt
